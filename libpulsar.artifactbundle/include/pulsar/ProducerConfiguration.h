...
public:
  ...
  enum ProducerAccessMode {
    ...
    /**
     * Acquire exclusive access for the producer. Any existing producer will be
     * removed and invalidated immediately.
     */
  ...
  /**
   * Set the producer name which could be assigned by the system or specified by
   * the client.
   *
   ...
   * subsequent messages are assigned incremental sequence IDs.
   *
   ...
   * <li>{@link CompressionZSTD}: Zstandard Compression
   * https://facebook.github.io/zstd/ (Since Pulsar 2.3. Zstd cannot be used if
   * consumer applications are not in version >= 2.3 as well)</li> <li>{@link
   * CompressionSNAPPY}: Snappy Compression  https://google.github.io/snappy/
   * (Since Pulsar 2.4. Snappy cannot be used if consumer applications are not
   * in version >= 2.4 as well)</li>
   * </ul>
   */
   ...
   * acknowledgment from the broker. <p> When the queue is full, by default, all
   * calls to Producer::send and Producer::sendAsync would fail unless
   * blockIfQueueFull is set to true. Use {@link #setBlockIfQueueFull} to change
   ...
   */
  ProducerConfiguration &
  setPartitionsRoutingMode(const PartitionsRoutingMode &mode);
   ...
   */
  ProducerConfiguration &
  setMessageRouter(const MessageRoutingPolicyPtr &router);
  ...
  /**
   * Set the hashing scheme, which is a standard hashing function available when
   * choosing the partition used for a particular message.
   *
   * Default: HashingScheme::BoostHash
   *
   * <p>Standard hashing functions available are:
   * <ul>
   * <li>{@link HashingScheme::JavaStringHash}: Java {@code String.hashCode()}
   * (Default). <li>{@link HashingScheme::BoostHash}: Use [Boost hashing
  ...
  /**
   * This config affects producers of partitioned topics only. It controls
   * whether producers register and connect immediately to the owner broker of
   * each partition or start lazily on demand. The internal producer of one
   * partition is always started eagerly, chosen by the routing policy, but the
   * internal producers of any additional partitions are started on demand, upon
   * receiving their first message. Using this mode can reduce the strain on
   * brokers for topics with large numbers of partitions and when the
   * SinglePartition routing policy is used without keyed messages. Because
   * producer connection can be on demand, this can produce extra send latency
  ...
  /**
   * @return whether Producer::send or Producer::sendAsync operations should
   * block when the outgoing message queue is full. (Default: false)
   */
  ...
  /**
   * Control whether automatic batching of messages is enabled or not for the
   * producer.
   *
   * Default: true
   *
   * When automatic batching is enabled, multiple calls to Producer::sendAsync
   * can result in a single batch to be sent to the broker, leading to better
   * throughput, especially when publishing small messages. If compression is
   * enabled, messages are compressed at the batch level, leading to a much
   * better compression ratio for similar headers or contents.
   *
   * When the default batch delay is set to 10 ms and the default batch size is
   * 1000 messages.
   *
  ...
  /**
   * Return the flag whether automatic message batching is enabled or not for
   * the producer.
   *
   * @return true if automatic message batching is enabled. Otherwise it returns
   * false.
   * @since 2.0.0 <br>
   ...
   * 1000.</i> If you set this option to a value greater than 1, messages are
   * queued until this threshold is reached or batch interval has elapsed.
   *
   * All messages in a batch are published as
   *      a single batch message. The consumer is delivered individual messages
   * in the batch in the same order they are enqueued.
   * @param batchMessagesMaxMessagesPerBatch max number of messages permitted in
   * a batch
   * @return
   */
  ProducerConfiguration &
  setBatchingMaxMessages(const unsigned int &batchingMaxMessages);
   ...
   * <i>Default value: 128 KB.</i> If you set this option to a value greater
   * than 1, messages are queued until this threshold is reached or batch
   ...
   *
   * @param batchingMaxPublishDelayMs max time for message publish delay
   * permitted in a batch.
   * @return
   */
  ProducerConfiguration &
  setBatchingMaxPublishDelayMs(const unsigned long &batchingMaxPublishDelayMs);
   ...
   *
   * At the time of producer creation, Pulsar client checks if there are keys
   * added to encryptionKeys. If keys are found, a callback getKey(String
   * keyName) is invoked against each key to load the values of the key.
   * Application should implement this callback to return the key in pkcs8
   * format. If compression is enabled, message is encrypted after compression.
   ...
   */
  ProducerConfiguration &setProperty(const std::string &name,
   ...
   * and publish them to broker separately in order. So, it allows client to
   * successfully publish large size of messages in pulsar.
   *
   * Set it true to enable this feature. If so, you must disable batching (see
   * setBatchingEnabled), otherwise the producer creation will fail.
   *
   * There are some other recommendations when it's enabled:
   * 1. This features is right now only supported for non-shared subscription
   * and persistent-topic.
   * 2. It's better to reduce setMaxPendingMessages to avoid producer accupying
   * large amount of memory by buffered messages.
   * 3. Set message-ttl on the namespace to cleanup chunked messages. Sometimes
   * due to broker-restart or publish time, producer might fail to publish
   * entire large message. So, consumer will not be able to consume and ack
   * those messages.
   *
   * Default: false
   *
  ...
  ProducerConfiguration &
  intercept(const std::vector<ProducerInterceptorPtr> &interceptors);
